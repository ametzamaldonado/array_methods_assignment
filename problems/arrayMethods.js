...
const sumArray = (nums) => {
  let sumOfArray = 0;
  nums.forEach((num) => sumOfArray += num);
  return sumOfArray;
}
...
const zeroCount = (nums) => {
  let zeroArray = [];
  nums.forEach((num) => num === 0 ? zeroArray.push(num) : -1);
...
const targetCount = (nums, target) => {
  let count = 0;
  nums.forEach(num => num === target ? count++ : num)
  return count
};
...
const counterObject = (items) => {
  let object = {};
  items.forEach((item) => object[item] ? object[item]++ : object[item] = 1);
  return object;
};
...
const tripledAndLessThan20 = (nums) => {
  // return nums.every(num => (num * 20) > 20); <--- why does this work!
